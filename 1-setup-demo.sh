...
if [[ ${continuescript,,} != "n" ]]; then
    ~/bin/az storage account create -n VALUEOF-UNIQUE-STORAGE-PREFIXdrupalstore -g ossdemo-appdev-paas -l eastus --sku Standard_LRS
    

fi
echo ".getting storage account connection string for VALUEOF-UNIQUE-STORAGE-PREFIXdrupalstore"
STORAGECONN=`~/bin/az storage account show-connection-string -n VALUEOF-UNIQUE-STORAGE-PREFIXdrupalstore -g ossdemo-appdev-paas`
...
B64STORAGENAME=`echo "VALUEOF-UNIQUE-STORAGE-PREFIXdrupalstore" | base64`
B64STORAGEKEY=`~/bin/az storage account keys list -n VALUEOF-UNIQUE-STORAGE-PREFIXdrupalstore -g ossdemo-appdev-paas --query [1].value -o tsv | base64`
